...
출력의 `~M/D`는 직접 커밋이면 커밋일, merge된 PR의 포함 커밋이면 PR 병합일입니다. 여러 커밋을 하나의 업무로 묶으면 가장 마지막 작업일을 표시합니다. 미완료 업무를 차주 보고로 이월할 때는 사용자에게 예상 종료일을 입력받으며, 차주 보고의 `~M/D`에는 이 예상 종료일을 표시합니다.
